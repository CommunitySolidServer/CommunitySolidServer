...
import { BooleanOrHandler } from '../../../../src/util/handlers/BooleanOrHandler';
import { StaticAsyncHandler } from '../../../util/StaticAsyncHandler';

describe('A BooleanOrHandler', (): void => {
  ...
  it('can handle the input if any of its handlers can.', async(): Promise<void> => {
    const handler = new BooleanOrHandler([ handlerFalse, handlerCanNotHandle ]);
  ...
  it('returns true if any of its handlers returns true.', async(): Promise<void> => {
    const handler = new BooleanOrHandler([ handlerFalse, handlerTrue, handlerCanNotHandle ]);
  ...
  it('returns false if none of its handlers returns true.', async(): Promise<void> => {
    const handler = new BooleanOrHandler([ handlerFalse, handlerError, handlerCanNotHandle ]);
