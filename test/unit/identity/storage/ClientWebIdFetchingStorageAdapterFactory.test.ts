...
import {
  ClientWebIdFetchingStorageAdapterFactory,
} from '../../../../src/identity/storage/ClientWebIdFetchingStorageAdapterFactory';
...
describe('A ClientWebIdFetchingStorageAdapterFactory', (): void => {
  const fetchMock: jest.Mock = fetch as any;
  let triples: Quad[];
  const id = namedNode('id');
  let source: Adapter;
  let sourceFactory: StorageAdapterFactory;
  let adapter: Adapter;
  let factory: ClientWebIdFetchingStorageAdapterFactory;
  ...
  beforeEach(async(): Promise<void> => {
    ...
    factory = new ClientWebIdFetchingStorageAdapterFactory(sourceFactory);
