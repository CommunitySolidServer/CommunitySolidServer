...
export class MetadataController {
  ...
  public serializeQuads(quads: Quad[]): Readable {
    return pipeStreamsAndErrors(streamifyArray(quads), new StreamWriter({ format: 'text/turtle' }));
  }
  ...
  public async parseQuads(readable: Readable): Promise<Quad[]> {
    return await arrayifyStream(pipeStreamsAndErrors(readable, new StreamParser({ format: 'text/turtle' })));
  }
}
