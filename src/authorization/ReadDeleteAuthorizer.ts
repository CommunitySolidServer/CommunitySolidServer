    ...
    for (const identifier of input.requestedModes.distinctKeys()) {
      if (input.requestedModes.hasEntry(identifier, AccessMode.delete) &&
        ...
        if (input.availablePermissions.get(identifier)?.read) {
          ...
          input.availablePermissions.get(this.identifierStrategy.getParentContainer(identifier))?.read) {
          ...
          input.availablePermissions.get(identifier)!.delete = false;
        }
      }
    }
