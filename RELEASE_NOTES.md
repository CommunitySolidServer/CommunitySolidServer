...
- In the `ResourceStore` interface, the return types of `setRepresentation`, `addResource`, `deleteResource` and `modifyResource` have been changed to `Promise<Record<string, RepresentationMetadata>>`.
